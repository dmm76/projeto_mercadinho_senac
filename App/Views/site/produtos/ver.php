        ...
        <div class="row g-4 align-items-start">
          <div class="col-12 col-lg-6">
            <div class="card border-0 shadow-sm product-hero">
              <div class="ratio ratio-4x3 bg-light d-flex align-items-center justify-content-center">
                <img id="productMainImage" src="<?= $h($baseImage) ?>"
                  alt="<?= $h($produto['nome'] ?? 'Produto') ?>" class="w-100 h-100 p-3">
              </div>
            </div>
          ...
          <div class="col-12 col-lg-6">
              ...
              <div>
                ...
                <?php if ($temPromo): ?>
                  <div class="text-muted">
                    <small class="text-decoration-line-through">R$
                      <?= number_format($precoVenda, 2, ',', '.') ?></small>
                  </div>
                <?php endif; ?>
              </div>
              <?php if ($estoque !== null): ?>
                <span class="badge bg-light text-dark border product-meta">Estoque:
                  <?= $h((string) $estoque) ?></span>
              <?php endif; ?>
            </div>
            ...
            <div class="d-flex flex-wrap gap-3 align-items-end mt-4">
              <form method="post"
                action="<?= Url::to('/carrinho/adicionar/' . (int) ($produto['id'] ?? 0)) ?>"
                class="d-flex flex-wrap align-items-end gap-3">
                <?= Csrf::input() ?>
                <div class="">
                  <label for="qtd" class="form-label mb-1">Quantidade</label>
                  <input id="qtd" name="quantidade" type="number" class="form-control"
                    value="<?= number_format($valorInicial, $step === 1 ? 0 : 3, '.', '') ?>"
                    min="<?= $step ?>" step="<?= $step ?>" style="max-width: 140px;">
                </div>
              ...
              <?php if ($clienteId !== null): ?>
                <form method="post" action="<?= Url::to('/favoritos/toggle') ?>" class="d-flex">
                  ...
                  <button type="submit"
                    class="btn btn-outline-danger btn-lg px-4 <?= $isFavorito ? 'active' : '' ?>">
                    <i
                      class="bi <?= $isFavorito ? 'bi-heart-fill' : 'bi-heart' ?> me-2"></i><?= $isFavorito ? 'Remover dos Favoritos' : 'Adicionar aos Favoritos' ?>
                  </button>
                </form>
              <?php else: ?>
            ...
            <?php if (!$isPesoVariavel): ?>
              <small class="text-muted d-block mt-3">Informe a quantidade desejada em unidades
                inteiras.</small>
            <?php else: ?>
              <small class="text-muted d-block mt-3">Produto por peso: ajuste a quantidade em passos de
                <?= str_replace('.', ',', (string) $step) ?> kg.</small>
            <?php endif; ?>
          </div>
        </div>
      </div>
