<?php /** app/Views/site/carrinho/index.php */ ?>
<!doctype html>
<html lang="pt-br">
<head>
  <meta charset="utf-8"/>
  ...
  <style>
    .cart-item-img{object-fit:cover; width:100%; height:140px; border-radius:.5rem;}
    @media (min-width:768px){.cart-item-img{height:160px;}}
    .cart-empty{max-width:480px;}
  </style>
</head>
<body>
            ...
            <li class="list-group-item py-3">
              <div class="row g-3 align-items-center">
                <div class="col-4 col-md-3 col-lg-2">
                  <img src="<?= htmlspecialchars($imagemUrl) ?>" alt="<?= htmlspecialchars($produto['nome'] ?? 'Produto') ?>" class="cart-item-img shadow-sm"/>
                </div>
                <div class="col-8 col-md-5 col-lg-6">
                  <h4 class="h5 mb-1">
                    <a class="text-decoration-none text-danger" href="<?= \App\Core\Url::to('/produtos/' . (int)$produtoId) ?>">
                      <?= htmlspecialchars($produto['nome'] ?? 'Produto sem nome') ?>
                    </a>
                  </h4>
                  <p class="text-muted mb-2">
                    <?= htmlspecialchars($produto['unidade_sigla'] ?? '') ?> <?= !empty($produto['peso_variavel']) ? '(por peso)' : '' ?>
                  </p>
                ...
                <div class="col-12 col-md-4 col-lg-4">
                  <form class="input-group input-group-sm mb-2" method="post" action="<?= \App\Core\Url::to('/carrinho/atualizar/' . (int)$produtoId) ?>">
                    <?= \App\Core\Csrf::input() ?>
                    <button class="btn btn-outline-dark js-qty" type="button" data-step="<?= $step ?>" data-target="qty-<?= (int)$produtoId ?>" data-dir="-1">
                      <i class="bi bi-caret-down"></i>
                    </button>
                    <input type="number" class="form-control text-center" id="qty-<?= (int)$produtoId ?>"
                           name="quantidade" value="<?= number_format($quantidade, $step === 1 ? 0 : 3, '.', '') ?>"
                           min="<?= $step ?>" step="<?= $step ?>">
                    <button class="btn btn-outline-dark js-qty" type="button" data-step="<?= $step ?>" data-target="qty-<?= (int)$produtoId ?>" data-dir="1">
                  ...
                  <div class="d-flex justify-content-between align-items-center">
                    ...
                    <form method="post" action="<?= \App\Core\Url::to('/carrinho/remover/' . (int)$produtoId) ?>">
                      <?= \App\Core\Csrf::input() ?>
