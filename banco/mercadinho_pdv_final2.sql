...
-- Copiando estrutura para trigger mercadinho.trg_pagamento_insere_movcaixa
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_ZERO_IN_DATE,NO_ZERO_DATE,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER trg_pagamento_insere_movcaixa
AFTER INSERT ON pedido_pagamento
FOR EACH ROW
BEGIN
  -- somente para pedidos PDV
  IF (SELECT canal FROM pedido WHERE id=NEW.pedido_id) = 'pdv' THEN
    INSERT INTO mov_caixa (caixa_id, tipo, valor, descricao, pedido_id, terminal_id, turno_id)
    SELECT NULL, 'entrada', NEW.valor,
           CONCAT('Venda PDV #', NEW.pedido_id, ' (', NEW.tipo, ')'),
           NEW.pedido_id, meta.terminal_id, meta.turno_id
    FROM pdv_pedido_meta meta
    WHERE meta.pedido_id = NEW.pedido_id;
  END IF;
END//
DELIMITER ;
